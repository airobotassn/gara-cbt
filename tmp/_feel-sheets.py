...
files = sorted(glob.glob('tmp/_fz-*.png'))
S = 0.42
for a in range(0, 20, 5):
    ...
    for c, im in enumerate(ims):
        dr.text((10+c*(W+10), 8), f'구역 {a+c+1}', font=font, fill=(230, 200, 120))
        sheet.paste(im.resize((int(im.width*S), int(im.height*S)), Image.LANCZOS), (10+c*(W+10), 40))
    out = f'tmp/feel20-{a+1}-{a+5}.jpg'; sheet.save(out, quality=86); print(out, sheet.size)
