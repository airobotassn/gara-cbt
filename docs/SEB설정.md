...
CARIS 자격검정을 **보안 브라우저(SEB)** 로만 응시하게 잠그는 설정 절차다.
...
## 🚪 SEB 안에 갇혔을 때 (테스트 중 제일 먼저 필요한 것)

SEB는 뒤로가기·새로고침·주소창·앱전환이 전부 막혀 있다. 나가는 길은 셋뿐이고, **순서대로 시도한다.**

1. **화면의 `보안 브라우저 종료` 버튼** — 오른쪽 아래에 항상 떠 있다(응시 중 제외). 누르면 quitURL 로 가서 비밀번호 없이 닫힌다.
2. **버튼이 안 보이면**(앱이 아예 안 뜬 경우 등) `Ctrl+Q` → 종료 비밀번호 **`gara-exit-2026`**
