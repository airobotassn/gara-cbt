...
//  · 액션 4개: lookup(닉네임→상대 카드) · send(이체) · history(이력) · seen(받은 선물 확인 처리)
//  · cosmetic-only 하드 불변식: user_progress / user_level_skill 을 절대 만지지 않는다.
//    코인은 시즌 점수·랭킹과 완전히 별개 지갑이라 이 기능은 순위에 영향을 주지 않는다.
//
// ⚠️ **금액은 되돌릴 수 없다.** 즉시 이체라 취소·회수 경로가 없다. 방어선은 프론트가 보여주는
//    상대 카드 확인과 client_nonce 재사용 규약뿐이다 — nonce 를 호출마다 새로 만들면 재시도가 곧
//    두 번 보내기다.
// ⚠️ 상대를 **닉네임**으로 지목한다(2026-08-24, 옛 친구코드 경로는 제거). 닉네임이 유일하다는 게
//    전제이고 그 보장은 DB 인덱스(profiles_nickname_key_uniq)에 있다 — 여기서 다시 검사하지 않는다.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    //     익명은 보낼 수도 받을 수도 없다 — 받는 쪽은 닉네임을 확정한 적이 없어서(nickname_set_at null)
    //     조회·이체 양쪽 where 절에 걸리지 않는다.
    ...
    // ---------- 닉네임 → 상대 카드 ----------
    // ⛔ 이름만 돌려주면 안 된다. 사용자가 친 값이 그대로 되돌아오는 것이라 확인 기능이 0이다 —
    //    아바타·시즌점수·국가·지역까지 같이 줘서 **사용자가 안 친 정보**로 사람을 알아보게 한다.
    // 쿼터(10분 60회)는 RPC 안에 있다.
    if (action === 'lookup') {
      const { data, error } = await admin.rpc('coin_gift_lookup', {
        p_uid: uid,
        p_nick: String(body.nickname ?? ''),
      })
