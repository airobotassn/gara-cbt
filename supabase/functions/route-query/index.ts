...
const DEST: Record<string, string> = {
  ...
  daily: '/daily',              // DAILY QUIZ(옛 이름 '오늘의 학습')·오늘의 문제
  // --- CARIS 자격검정 ---
  ...
  // --- 내 정보 ---
  // ⚠️ `/mypage` 가 아니라 응시 현황 탭이다 — 2026-08-25 에 마이페이지 첫 탭이 이북 서재가 되면서
  //    "내 점수" 를 검색한 사람이 서재로 떨어졌다. 이 인텐트가 원래 말하는 건 응시 이력이다.
  mypage: '/mypage/attempts',   // 내 점수·결과·응시 이력
  ebook_library: '/mypage/ebooks', // 구매한 이북·강의 보기(마이페이지 '내 서재')
  ebook: '/ebooks',             // 러닝 라이브러리 — 이북·강의 사러 가기
  login: '/login',              // 로그인
  // --- 사이트 정보 ---
  about: '/about',              // 협회 소개
  notice: '/notice',            // 공지사항
