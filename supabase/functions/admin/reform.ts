...
//   ⚠️ 금액은 **달러 센트**다(`payments.amount` = 정가 센트). 이 주석이 한동안 "원화" 라고 적혀 있어서
//      화면이 `krw()` 로 찍었고 $1 이 "100원" 으로 나왔다(2026-09-16). 실제 청구액(`charge_amount`)을
...
// ⚠️ 표(`minigame_question_sets`)는 2026-09-04 에 드롭했다(20260904540000). 그때까지 "되살릴 일이
//    생기면" 하고 남겨뒀는데, **남겨둔 데이터가 되살릴 때 쓸모가 없어서**다:
//      · 되살리는 방법은 아래 한 줄이 이미 정해 뒀다 — 분야로 거는 쪽. 분야는 term_questions.field 에
//        이미 있어서 그 200줄을 안 쓴다.
//      · 담긴 내용도 낡았다 — `daily` 50줄은 가리킬 대상이 없어졌고(DAILY QUIZ 가 이 은행을 안 쓴다),
//        나머지 150줄은 "세 게임에 전부 같은 50문항" 이라 지금 동작과 똑같아 정보량이 0이었다.
// ⛔ 진짜 갈라야 할 일이 생기면 표를 다시 만들지 말고 **분야(AI·로봇·피지컬AI)** 로 걸 것.
//
