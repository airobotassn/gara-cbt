...
export async function listReports(admin: any, body: any) {
  let q = admin
    ...
    .select('id, code, question_id, message, status, lang, created_at, questions(level, category, prompt_i18n)')
  ...
  const reports = (data ?? []).map((r: any) => ({
    ...
    level: r.questions?.level ?? null,
    category: r.questions?.category ?? null,
    prompt: r.questions?.prompt_i18n?.ko ?? '(문항 삭제됨)',
  }))
  // 미처리 건수도 같이
