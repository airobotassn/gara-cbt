...
import { SUPPORTED_LANGS } from '../../_shared/lib.ts'
...
export async function analytics(admin: any) {
  ...
  const [profRes, attRes, ansRes, qRes, progRes, usersCntRes, guestsCntRes, attsCntRes, atts7dCntRes] = await Promise.all([
    ...
    admin.from('attempt_answers').select('question_id, category, is_correct').limit(50000),
    admin.from('questions').select('id, level, category, correct_index, prompt_i18n, options_i18n, active').is('deleted_at', null).limit(5000),
