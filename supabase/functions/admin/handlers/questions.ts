...
import { axisKeysForLevel, SUPPORTED_LANGS, MAX_LEVEL } from '../../_shared/lib.ts'
...
export async function upsertQuestions(admin: any, body: any, actor: string) {
  ...
  if (newRows.length) {
    ...
    const { data: ex } = await admin.from('questions').select('level, code').in('level', levels)
  ...
  const { data, error } = await admin.from('questions').upsert(payload).select('id')
  if (error) return json({ error: error.message }, 500)
...
export async function setActive(admin: any, body: any, actor: string) {
  ...
  const { data: before } = await admin.from('questions').select('code, level').eq('id', body.id).maybeSingle()
