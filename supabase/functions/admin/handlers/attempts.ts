...
import { pickLang, projText, projOptionsForLevel } from '../../_shared/lib.ts'
...
export async function attemptDetail(admin: any, body: any) {
  ...
  const { data: rows } = await admin
    ...
    .select('category, selected_index, is_correct, questions(prompt_i18n, options_i18n, correct_index)')
    .eq('attempt_id', aid)
  const answers = (rows ?? []).map((r: any) => ({
    category: r.category,
    prompt: projText(r.questions?.prompt_i18n, lang),
    options: projOptionsForLevel(r.questions?.options_i18n, lang, level),
