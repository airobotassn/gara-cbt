...
const SEED: { dest: string; phrases: string[] }[] = [
  ...
  {
    dest: '/hub', // 캐릭터 허브: 캐릭터·아바타·코인·상점·출석
    phrases: [
      ...
      '角色中心', '我的角色', '更换头像', '金币', '我的金币', '商店', '购买道具', '签到', '大厅',
      'कैरेक्टर हब', 'मेरा कैरेक्टर', 'अवतार बदलें', 'सिक्के', 'दुकान', 'उपस्थिति',
      'trung tâm nhân vật', 'nhân vật của tôi', 'đổi ảnh đại diện', 'xu của tôi', 'cửa hàng', 'điểm danh',
    ],
  },
  {
  ...
  {
    dest: '/daily', // DAILY QUIZ(옛 이름 '오늘의 학습')·오늘의 문제
    ...
    //    ⚠️ 2026-08-27 이 화면 이름이 '오늘의 학습' → **DAILY QUIZ** 로 바뀌었다(6개국어 같은 글자).
    //       옛 이름 앵커는 그대로 둔다 — 그 이름으로 검색하던 사람이 못 찾게 되면 안 된다.
    phrases: [
      'DAILY QUIZ', '데일리 퀴즈', '데일리퀴즈',
      ...
      '今日の学習', '今日の問題', 'デイリー', '毎日の学習', '今日のクイズ', 'デイリークイズ',
      '今日学习', '今日一题', '每日内容', '每日任务', '每日测验',
      'आज की पढ़ाई', 'आज की समस्या', 'दैनिक सामग्री', 'डेली क्विज़',
      'bài học hôm nay', 'bài toán hôm nay', 'nội dung hằng ngày', 'câu đố hằng ngày',
    ],
  },
  {
  ...
  {
    // ⚠️ `/mypage` 가 아니라 응시 현황 탭이다 — 마이페이지 첫 탭이 이북 서재가 되면서(2026-08-25)
    //    "내 점수" 검색이 서재로 떨어졌다. 이 블록의 문구는 전부 응시 이력을 찾는 말이다.
    dest: '/mypage/attempts', // 내 점수·시험 결과·응시 이력
    phrases: [
  ...
  },
  {
    ...
    phrases: [
      // ⚠️ 의견·건의도 여기로 보낸다(2026-08-25 결정). 의견함(/feedback)은 FAB 전용 화면이라
      //    검색 목적지로 만들지 않는다 — 문의와 의견을 벡터로 가르기도 어렵다.
      '문의하고 싶어', '물어볼 게 있어', '고객센터', '환불 어떻게 해', '결제 문제',
      '도움이 필요해', '자주 묻는 질문', '시스템 오류', '채점 문의', '기업 단체 문의',
      '연락처 알려줘', '상담하고 싶어', '문의사항 있어', '문제가 생겼어',
      '의견 보내기', '건의사항', '피드백 남기고 싶어', '개선 요청',
      'I have a question', 'contact support', 'customer service', 'how to get a refund', 'payment issue',
      'I need help', 'faq', 'system error', 'corporate inquiry', 'send feedback', 'suggestion',
      '問い合わせたい', 'カスタマーサポート', '返金したい', '支払いの問題', '困っています', 'ご意見を送りたい',
      '我想咨询', '客服', '怎么退款', '支付问题', '需要帮助', '提意见', '建议',
      'मुझे पूछताछ करनी है', 'ग्राहक सेवा', 'रिफंड कैसे लें', 'भुगतान समस्या', 'मदद चाहिए', 'सुझाव भेजें',
      'tôi muốn hỏi', 'chăm sóc khách hàng', 'hoàn tiền thế nào', 'vấn đề thanh toán', 'cần trợ giúp', 'gửi góp ý',
    ],
  },
  {
    // 러닝 라이브러리 = 사러 가는 곳. 이미 산 걸 보는 곳은 아래 /mypage/ebooks.
    // ⚠️ **강의도 여기서 판다**(2026-08-25). 그 전까지 이 블록에 강의 문구가 한 개도 없어서
    //    "강의 사고싶어" 가 어디로 갈지 아무도 보장 못 했다.
    dest: '/ebooks',
    phrases: [
      '이북', '전자책', '이북 사고 싶어', '교재 사기', '책 구매', '전자 교재', '교재 어디서 사', '이북 스토어',
      '강의', '인강', '강의 사고 싶어', '강의 구매', '온라인 강의', '강좌 어디서 사', '러닝 라이브러리',
      'ebook', 'e-book', 'buy an ebook', 'digital book', 'ebook store', 'where to buy textbooks',
      'lecture', 'buy a lecture', 'online course', 'video lessons', 'where to buy courses',
      '電子書籍', 'eBookを買いたい', '教材を買う', '電子書籍ストア', '講義', '講義を買いたい', 'オンライン講座',
      '电子书', '想买电子书', '教材购买', '电子书商店', '课程', '想买课程', '网课',
      'ईबुक', 'ईबुक खरीदना है', 'डिजिटल किताब', 'ईबुक स्टोर', 'लेक्चर', 'ऑनलाइन कोर्स',
      'sách điện tử', 'mua ebook', 'giáo trình điện tử', 'cửa hàng ebook', 'bài giảng', 'mua khóa học',
    ],
  },
  {
    // 이미 산 이북·강의를 보는 곳(마이페이지 '내 서재' 탭 — 2026-08-25 에 '이북 서재'에서 이름이 바뀌었고
    // 강의도 여기 들어온다). URL 은 그대로 /mypage/ebooks 다.
    dest: '/mypage/ebooks',
    phrases: [
      '내 이북', '이북 서재', '내 서재', '구매한 책 어디서 봐', '산 책 읽기', '구매한 이북 읽기', '내가 산 교재',
      '결제한 책 어디 있어', '구매한 교재 보기', '내 강의', '구매한 강의 어디서 봐', '산 강의 보기',
      'my ebook library', 'where can I read my book', 'my purchased ebooks', 'read my ebook', 'my bookshelf',
      'my lectures', 'where to watch my course',
      '私の電子書籍', '購入した本はどこ', '本棚', '買った本を読む', '購入した講義',
      '我的电子书', '购买的书在哪看', '书架', '阅读已购图书', '我的课程',
      'मेरी ईबुक', 'मेरी खरीदी किताबें', 'मेरी किताबें कहाँ पढ़ें', 'मेरे कोर्स',
      'thư viện ebook của tôi', 'sách đã mua ở đâu', 'đọc sách đã mua', 'khóa học của tôi',
    ],
  },
  {
    dest: '/login', // 로그인
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (reset) {
      // ⛔ **학습분(source='llm')도 같이 지운다.** 예전엔 seed 행만 지웠는데, 검색이 맞힌 답을
      //    저장하는 write-back 은 llm 으로 들어간다 → 목적지를 고치고 재시드해도 예전에 학습된
      //    질의는 **옛 페이지로 계속 갔다**(2026-08-25 점검에서 드러난 구멍).
      //    학습분은 시드에서 다시 자라는 것이라 버려도 잃는 게 없다.
      await supabase.from('route_cache').delete().in('source', ['seed', 'llm'])
    }
