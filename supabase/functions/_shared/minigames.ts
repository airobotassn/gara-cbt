...
//   'level' — 레벨제 퍼즐(닿아라·프로그램해라·지어라·시켜라·더듬어라). 도달 레벨이 1차 키, **동률은 소요시간(tieMs) 짧은 순**
//     (프로그램해라·지어라는 시간 대신 명령/타일 수 합 → 실행/가동 횟수 합, 시켜라는 별 합 → 시간 — 서버가 log 로 센다).
//     레벨 수가 적은 게임은 도달 레벨만으로는 전원 만점이 되어 보드가 무의미해진다 → 동률 키가 필요하다.
//
// replay: 답안 기록 재채점(버텨라·쏴라·골라라·닿아라 — 2026-09-11 · 프로그램해라·지어라·막아라 — 09-14/15). 점수를 받지 않고 기록을 받아 서버가 다시 센다(./minigame-replay.ts).
...
export const GAMES: Record<string, GameSpec> = {
  ...
  'build-cari': { metric: 'level', replay: 'build' }, // 깬 레벨 수(30) — 동률은 타일 수 합 → 가동 횟수 합(서버가 log 의 배치를 시뮬레이션해 다시 센다)
  'program-cari': { metric: 'level', replay: 'program' }, // 깬 레벨 수 — 동률은 명령 수 합 → 실행 횟수 합(서버가 log 로 센다)
  // 막아라 = 서류를 읽고 규정 위반을 가리는 점수형(10일 × 5장). 기록 재채점 — "뭘 가리고 어느 도장을 찍었나" 를 서버가
  //   _shared/block-days.ts 로 다시 판정한다(2026-09-15). 만점은 저절로 정해진다(전송형 42장 × 150 + 반려형 8장 × 100 = 7,100).
  //   ⚠️ 옛 값 max 3000·perSec 75 는 20장 시절 것 — 재채점이라 더는 안 쓴다.
  'block-cari': { metric: 'score', replay: 'block' },
