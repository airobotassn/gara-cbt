-- 문항 이력 옛 표 3개 드롭 — `20260904400000_question_history_merge.sql` 의 마지막 단계(④).
--
-- ⛔ **함수를 배포하기 전에는 적용하지 말 것.** 브라우저와 엣지 함수가 컬럼을 이름으로 select 하므로,
--    배포 전 코드가 살아 있는 동안 지우면 PostgREST 가 400 을 내고 **관리자 이력 탭 3개가 통째로 멈춘다.**
--    순서는 ① 20260904400000 적용 → ② `supabase functions deploy admin admin-test`
--          → ③ 20260904400000 재실행(틈 메우기) → ④ **이 파일**.
--
