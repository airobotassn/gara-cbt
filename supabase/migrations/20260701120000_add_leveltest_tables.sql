-- ============================================================
-- SEMI-CARIS 통합 — SEMI-CARIS 테이블을 CBT 프로젝트에 추가(네임스페이싱)
...
--
-- [겹침 처리 요약]
--  · questions(SEMI-CARIS) → test_questions  로 리네임 (CBT questions 와 이름 충돌 회피)
--  · attempt_answers(SEMI-CARIS) → test_answers 로 리네임 (CBT attempt_answers 와 충돌 회피)
--  · test_attempts / user_level_skill / user_progress / question_reports / reco_* : 충돌 없어 그대로
--  · profiles.is_anonymous : CBT 정리 때 제거됐던 걸 되살림 (SEMI-CARIS 게스트 응시에 필요).
...
-- ---------- attempt_status enum (SEMI-CARIS test_attempts 용) ----------
do $$ begin
...
-- ---------- test_attempts (SEMI-CARIS 응시 = 누적 레이팅 갱신 + 등급 이동) ----------
