...
--  · 파츠/상점/쇠퇴/스탬프/티켓/출석은 순수 꾸미기·재화 레이어일 뿐,
--    레벨(진화 모양)·자격증(칭호)·실력 데이터는 절대 건드리지 않는다.
--  · 가격/임계 등 수치는 카탈로그 표가 권위 — DDL 에 하드코딩 금지.
...
-- 보유 파츠(꾸미기 아이템). source = shop | grant 등.
...
-- Phase2 경제 — 서버권위 상점(shop) 원자 트랜잭션.
...
-- ⛔ 옛 Phase 4 Lecture AI(강의별 RAG + 엔타이틀먼트 + 하루 질문 쿼터)는 2026-09-07 에 통째로 지웠다.
--    표 3개(lecture_chunks·lecture_entitlements·lecture_quota) · RPC 3개(match_lecture_chunks·
--    is_entitled·consume_quota) · 엣지 함수 lecture-qa · 테스트 2벌(t-lecture·lecture-qa-order).
--    한 번도 안 돌았다: 표 3개가 전부 0행이고, lecture-qa 를 부르는 화면 코드가 0곳이었으며,
--    셋 다 lecture_id 가 **text** 인데 실제 강의 id 는 **uuid** 라 애초에 이어지지 않는 구조였다.
--    ⛔ 되살릴 거면 표를 복구하지 말고 uuid 기준으로 새로 짤 것(2026-09-07 지시).
--    `create extension vector` 는 reco_cache·kb_chunks·route_cache 가 계속 쓰므로 아래에 남아 있다.
create extension if not exists vector;
-- ============================================================
...
select jsonb_build_object(
  'top', coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'rating', r.season_total,
      'avatar', pr.avatar_url,
      -- country: 시상대 이름 뒤 국기(20260818130000). base 가 범위 필터용으로 이미 뽑아 둔 값이다.
      --   me 행에는 안 넣는다 — uid 와 같은 이유(본인이라 이미 안다).
...
-- ============================================================
-- 아레나 집계 버킷 스냅샷 (20260818140000_arena_bucket_snapshot.sql)
--   ⚠️ 위쪽 '집계 리더보드 RPC' 절의 region_/country_/school_leaderboard 정의를 **여기서 덮어쓴다**.
--      위 정의는 매 요청 재계산판이고, 지금 도는 건 아래 스냅샷 조회판이다(schema.sql 은 위에서
--      아래로 실행되므로 나중 정의가 이긴다). 위 블록을 참고할 때 이 절을 같이 볼 것.
--   시드 데이터는 20260818140100_arena_seed_data.sql (생성물 — tools/gen-arena-seed.mjs).
--   크론 등록(pg_cron)은 마이그레이션에만 있다 — schema.sql 은 확장을 건드리지 않는다.
-- ============================================================
