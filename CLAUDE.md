...
- **랭킹 더미(`ranking_dummies`) — /ranking 이 비어 보이지 않게(2026-08-21)**: 지역 3,504개 × 10명 = **3만5천 명의 가상 랭커**. 고치려던 어긋남 둘 = ① 월드 랭킹이 전부 한국인이고 점수도 낮다 ② /arena 지도는 "이 지역에 회원이 있다" 는데 그 지역 사람이 랭킹을 열면 아무도 없다.
  ...
  - **닉네임은 나라별 문자 + 순위별 결이다(2026-08-21 개편, `20260821150000`)** — 한국은 한글, 일본은 가나·한자, 중국·대만은 한자, 인도·베트남은 그쪽 이름+게임 태그, 나머지는 영문. 첫 판은 `ZenHawk36` 처럼 **형용사+명사+두자리**가 100% 였는데, 랭킹은 열 줄이 세로로 붙는 화면이라 **패턴이 하나면 그게 그대로 보인다**(사람이 아니라 생성물로 읽힌다).
    ...
    - ⚠️ **캐릭터는 전원 갖는다.** 실회원 중에는 미선택이 실제로 있다(캐릭터 선택은 `/hub` 첫 진입에서만 강제되는데 레벨테스트만 응시해도 점수가 생겨 랭킹에 선다 — 실측 8명 중 3명). 처음엔 그걸 닮으려고 5분의 1을 비웠는데, 비면 그 자리가 **전부 같은 기본 그림**이라 랭킹·남의 방에서 그것만 눈에 띈다. 스킨(밤 배경)은 코인 상품이라 3분의 1만 갖는다.
  ...
  - 검증: `tests/db/t-ranking-dummies.mjs`(69건 — ⭐천장·⭐profiles 무오염·⭐TOP10 나라 다양성·⭐닉네임·⭐커서로 끝까지 걸어 누락 0·첫 화면과 맞물림·⭐허브와 랭킹이 같은 순위·결정론·걷어내기).
...
- **CARIS 자격검정 문항 다국어(2026-08-25)** — 응시자가 자기 언어로 시험을 본다. 여태 `questions.prompt`(text)·`choices`(jsonb) 단일 컬럼이라 번역을 담을 자리가 없어서, 무료 레벨테스트는 6개국어인데 **돈 받는 CARIS 만 한국어**였다.
  ...
  - **번역은 관리자가 누른다**(레벨테스트와 같은 파이프라인 = `lib/adminTranslate.ts` → `translate-questions`). 자리는 셋: 문항 목록의 `🌐 미번역 번역`(선택분 또는 미번역 전부) · 문항 편집 모달의 `🌐 자동 번역` · 엑셀 업로드 후 안내. ⚠️ **업로드 직후 자동 번역은 안 한다** — 수백 문항이면 관리자가 몇 분간 창을 못 닫고, 중간에 닫으면 어디까지 됐는지 안 남는다.
  ...
  - ⚠️ **해설(`explanation`)·모범답안(`answer_key`)은 번역하지 않는다** — 관리자 전용이라 응시자에게 안 나간다(해설은 번역기에 **문맥으로 보내기만** 하고 받은 번역은 버린다).
  - ⛔ **주관식 채점은 아직 한국어 전제다**(2026-08-25 보류). Elite 주관식 10문항을 외국어로 쓰면 관리자가 한국어 모범답안과 대조할 수 없다 — 실제 외국인 응시자가 생기면 **채점 화면에 답안 번역을 붙이는 것**이 답이다(모범답안만 번역하는 건 도움이 안 된다).
