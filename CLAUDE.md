...
- **관리자 '통계' 대메뉴 · 방문 로그 (2026-09-09 · `20260909120000`)** — 숫자 화면이 제품마다 흩어져 있어서(홈·유저관리·아레나·CARIS) "회원이 늘고 있나", "돈은 얼마나 버나" 에 답하려면 화면 서넛을 오가야 했다. **주제로** 모은 대메뉴를 새로 만들었다: 전체 개요 / 접속·유입 / 회원 / 학습·참여 / 자격검정 / 매출.
  ...
  - **IP주소별은 대역까지만 본다 (2026-09-09 지시)** — `185.93.89.147` 이 아니라 **`185.93.89.*`**(IPv6 는 앞 3그룹). "어느 통신사·회사망에서 몰려오나" 는 그대로 읽히고 개인 특정만 빠진다.
    - ⛔ **원문을 저장하지 말 것.** 마스킹은 `track-visit` 의 `maskIp` 하나에서 끝나고, `visit_log.ip_masked` 에 **형태 CHECK**(끝이 `*`)가 걸려 있어 원문은 표에 못 앉는다. 적재 함수도 원문이면 그 칸만 비우고 줄은 남긴다(거절하면 방문 기록 전체가 안 쌓인다).
    - ⛔ **가려도 보유기간·파기 의무는 그대로다** — 그래서 크론 `visit-history-purge`(`purge_visit_history`, 매일 03:40 KST)가 같이 들어갔다. **개인정보처리방침의 "180일 보관 후 파기"(집계는 400일)와 이 숫자는 한 벌**이라 한쪽만 고치면 안 된다. 방침에 적어놓고 안 지우면 그 자체가 위반이다.
    - ⚠️ 재동의는 안 받았다(2026-09-09 지시). 방침 제2조 자동수집 항목에 **'접속 IP 정보' 가 이미 있었고**, 이번 변경은 그걸 대역으로 **좁혀서** 적은 것이다.
