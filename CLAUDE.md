...
- **재진입 감지는 `start-exam` 이 한다.** SEB 는 사용자가 X 를 눌렀다고 알려주지 않지만, 다시 들어올 땐 반드시 이 함수를 또 탄다 — 서버가 확실히 아는 유일한 사실이다. 진행중 응시에 두 번째 진입이 오면 `voided`(`void_reason='reentry'`) + 409 `reentry_voided`.
...
- ⛔ **복구는 시계를 멈춘 상태로 돌려준다.** `elapsed_sec`(중단 시점까지 쓴 시간)만 남기고, **응시자가 실제로 다시 들어오는 순간** `start-exam` 이 `started_at = 그때 − 쓴 시간` 으로 옮긴다. 복구 시점에 맞추면 안 된다 — 관리자가 눌러놓고 응시자가 몇 시간 뒤 들어오면 그 사이가 다 흘러간다. 기준은 `last_seen_at` 이다(`submitted_at` 은 '나갔다 돌아온 순간' 이라 나가 있던 시간까지 쓴 것으로 계산된다).
...
- ⚠️ 복구는 새 응시가 아니다 — 문항 세트는 그대로고 **이미 쓴 시간은 돌려주지 않는다**(남은 시간만 복원). "처음부터 다시" 가 필요하면 응시권을 새로 발급해야 한다.
- **응시자 안내에 세 가지를 반드시 담는다**(`seb.voided_how`): ① 남은 시간이 보존된다 ② 응시 기간이 끝나도 별도 기한을 준다 ③ 확인에 영업일이 걸린다. **24시간 상주 운영이 아니므로 즉시 처리를 약속하지 않는다.**
