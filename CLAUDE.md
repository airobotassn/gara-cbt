...
- **첫 진입 흐름 · 캐릭터 · 스킨(2026-08-20)** — 신규 가입자가 `/hub` 에 처음 들어오면 **캐릭터를 고르고 → 튜토리얼을 본다.** 그 뒤로는 **꾸미기**(옛 '상점')에서 캐릭터·배경 스킨을 코인으로 사서 보관함에서 갈아입는다. 갈아입은 모습은 허브·공유 카드·랭킹/채팅 카드·남의 방에 **전부 같이 반영**된다.
  ...
  - **배경은 세 장 — 기본 초원 + 고궁 낮·밤(2026-08-25 개편 · `20260825130000`).** 기본은 **초원**(`skin_meadow`, 값 0 · 비판매)이고 고궁 낮·밤은 **각각 300코인 상품**이다. 고궁 둘은 UI 한 벌을 공유하고 배경만 다르며, 초원은 아무 그림도 안 얹는 **옛 카툰 CSS 한 벌**을 그대로 쓴다.
    ...
    - ⚠️ **레일 칸(꾸미기·칭호·초대하기)은 정사각형이다(2026-08-25 지시).** '한 화면에 고정' 블록이 칸을 `flex:1 1 0` 로 무대 높이만큼 늘려서, 그냥 두면 폭 64px 짜리가 세로 200px 넘는 기둥이 되어 버튼이 아니라 띠로 읽힌다. `[data-ui='base']` 가 늘어나기를 끄고 `aspect-ratio:1` 을 박은 뒤 레일을 위에서부터 쌓는다. ⚠️ 칸 간격은 16px 아래로 내리지 말 것 — '초대하기'의 EVENT 배지가 칸 위로 11px 튀어나와 위 칸 바닥에 얹힌다.
...
- **`/hub` 화면 구성(2026-08-04 시안 반영)**: 상단 HUD(아바타 · 이름 + 티어 **엠블렘** · **ARENA 레벨 경험치 바** + `?` + 코인) → **오늘의 미션 바**(한 줄) → 캐릭터 무대(+ 오른쪽 레일 3칸 `꾸미기·칭호·초대하기`) → 도크(7일 출석 스탬프 + 랭킹 CTA). 전부 전체 폭 세로 스택이라 캐릭터가 화면 정중앙에 온다.
  ...
  - **친구 초대는 도크 `초대하기` 모달 하나에서 다 끝난다** — 화면에 카드로 꺼내지 않는다(진입점 중복 제거). 모달 = 위(내 코드 + 복사) + 아래(친구 코드 입력).
    ...
    - **보상 = 코인 50, 초대한 쪽·코드를 쓴 쪽 양쪽 다(2026-08-24 변경).** 옛 보상(초대자에게만 시즌 점수 +5)은 제거됐다 — 위 '시즌 점수' 절 참고.
      - **초대자 쪽은 상한이 없다(지시).** 초대할수록 계속 50코인을 받는다. 피초대자 쪽은 `referred_by` 가 1회용이라 계정당 한 번이고, 그게 이 기능의 **유일한 자연 상한**이다.
        ⚠️ 그래서 부계정으로 코인을 찍는 길이 열려 있다(20개면 1,000코인). 코인은 스킨을 사는 지갑이라 문제가 되면 상한을 얹을 것 — 원장이 따로 없어도 `profiles.referred_by` 를 세면 몇 명을 초대했는지 나온다.
      ...
      - 금액의 단일 출처 = 마이그레이션 `20260824130000_referral_coin.sql` 의 `c_coin`. 화면(`Hub.tsx` 의 `REFERRAL_COIN`)은 안내표용 사본이고, 성공 문구의 금액은 **서버가 준 값**을 쓴다.
...
- **랭킹 더미(`ranking_dummies`) — /ranking 이 비어 보이지 않게(2026-08-21)**: 지역 3,504개 × 10명 = **3만5천 명의 가상 랭커**. 고치려던 어긋남 둘 = ① 월드 랭킹이 전부 한국인이고 점수도 낮다 ② /arena 지도는 "이 지역에 회원이 있다" 는데 그 지역 사람이 랭킹을 열면 아무도 없다.
  ...
  - 대가는 **더미가 합류할 자리를 손으로 연 것** 셋: `scoped_top`(pool CTE 로 union — 순위·백분위·티어는 여전히 SQL 이 계산해서 **화면 코드는 하나도 안 바뀌었다**) · `room` 함수 `view`(남의 방 보기) · `leaderboard` 의 캐릭터 조인·남의 카드 국가/지역 순위.
  ...
  - 검증: `tests/db/t-ranking-dummies.mjs`(63건 — ⭐천장·⭐profiles 무오염·⭐TOP10 나라 다양성·⭐닉네임·⭐커서로 끝까지 걸어 누락 0·첫 화면과 맞물림·결정론·걷어내기).
...
- **시즌 점수 (2026-08-04 원안 반영)**: 리더보드 정렬 단일 출처 = `user_progress.season_total` = **레벨테스트 트랙**(`skill_score`) + **활동 트랙**(`activity_score`).
  ...
  - ⛔ **친구 초대는 이제 점수를 주지 않는다(2026-08-24)** — 보상이 **코인 50(양쪽 다)** 으로 옮겨갔다(아래 '친구 초대' 절). 그래서 활동 상한이 6,570 → **4,745**, 전체 상한이 13,570 → **11,745** 로 내려갔다.
