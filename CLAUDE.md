...
- **첫 진입 흐름 · 캐릭터 · 스킨(2026-08-20)** — 신규 가입자가 `/hub` 에 처음 들어오면 **캐릭터를 고르고 → 튜토리얼을 본다.** 그 뒤로는 **꾸미기**(옛 '상점')에서 캐릭터·배경 스킨을 코인으로 사서 보관함에서 갈아입는다. 갈아입은 모습은 허브·공유 카드·랭킹/채팅 카드·남의 방에 **전부 같이 반영**된다.
  ...
  - **값 = 캐릭터 500 · 배경(밤) 300 코인이고, 첫 선택은 값과 무관하게 공짜다(2026-08-24 결정 · `20260824120000`).** 신규 회원은 **진열 중인 캐릭터 아무거나 한 종**을 공짜로 받고, 값은 **두 번째 캐릭터부터** 붙는다. 코인 수입은 하루 1회 10코인(출석·학습 통틀어)이 유일해서 500 ≒ 50일치 · 300 ≒ 30일치다.
  ...
  - **배경은 두 장 — 같은 고궁의 낮·밤(2026-08-20).** 낮이 기본(값 0, 전원 사용)이고 밤이 코인 상품이다. 둘은 UI 한 벌을 공유하고 배경만 다르지만, **그건 이 둘만의 예외**다 — 새 스킨은 자기 값 블록을 통째로 갖고 판·게이지·도장·아이콘까지 바꾼다.
