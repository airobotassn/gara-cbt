...
- 라이브: **https://garacaris.com** (2026-09-21 도메인 연결 — DNS 는 Cloudflare, 등록은 카페24. Cloudflare worker 이름 = wrangler.jsonc 의 `gara-cbt`. 옛 `gara-cbt.airobotassn.workers.dev` 도 그대로 열리고 Supabase 허용목록·결제 복귀 목록에 남아 있다)
...
| `/mypage` · `/mypage/:section` | `pages/MyPage.tsx` | (Tailwind 유틸) | `my-attempts` · `ebooks` |
...
| `/pay/success` (결제 결과 — 성공·실패 둘 다) | `pages/PayResult.tsx` | (Tailwind 유틸) | `payments` |
| **캐릭터 허브 / 미니게임** ||||
...
- **시즌 점수 (2026-09-03 개정표 · `20260903130000`)**: 리더보드 정렬 단일 출처 = `user_progress.season_total` = **레벨테스트 트랙**(`skill_score`) + **활동 트랙**(`activity_score`).
  ...
  - ⭐ **적립값의 단일 출처는 `reward_policy` 표다 (2026-09-07 지시 · `20260907200000`).** 관리자 › 적립 정책에서 바꾸면 **실제 적립이 바뀐다.** 읽는 자리는 `_shared/reward-policy.ts` 하나이고 `complete-daily`·`submit-minigame`·`get-hub` 가 그걸 부른다(화면의 적립 안내도 같은 값을 받아 그린다).
    - ⛔ **적립값 상수를 함수 안에 다시 두지 말 것.** 여태 이 표는 **채점에 안 쓰이는 사본**이었고, `complete-daily` 안의 또 다른 사본(`ACTIVITY_DELTA_SYNCED` — 이름과 달리 동기가 안 됐다)이 두 달간(8/4~9/4) 규격의 절반만 적립했다. 값이 두 벌이면 언제든 다시 갈린다. 검증 = `tests/db/t-reward-policy.mjs`(28건 — 사본 재발·폴백 일치·범위·꺼짐 처리까지 본다).
    - ⚠️ **못 읽으면 폴백이지 0 이 아니다.** DB 조회가 실패하거나 행이 없으면 `REWARD_FALLBACK`(= `scoring.ts` 의 규격값)으로 적립을 계속한다 — 0 으로 떨어뜨리면 사용자가 이유도 모른 채 점수를 잃는다. 반대로 `active=false` 는 관리자가 일부러 끈 것이라 그때만 0 이다.
    - ⚠️ **`_shared/reward-policy.ts` 는 `scoring.ts` 를 import 하지 않는다** — `complete-daily` 가 이걸 쓰는데 그 함수엔 cosmetic-only 불변식(scoring.ts import 금지)이 걸려 있다(`tests/cosmetic-only-gate.mjs`). 그래서 폴백 숫자를 그 파일이 직접 들고 있고, 위 테스트가 규격표와 대조한다.
    - ⛔ **범위 상한이 저장·적립 양쪽에 있다**(`REWARD_MAX_DELTA` 100 · `REWARD_MAX_PER_DAY` 50). 출석을 10 → 1000 으로 잘못 넣으면 시즌 상한이 무너져 **아레나 레벨 밴드(16,125 기준)가 통째로 틀어지고**, 되돌려도 이미 오른 레벨은 안 내려간다.
    - ⛔ **못 지키는 줄은 표에서 뺐다** — 친구 초대(`score/referral`, 보상이 코인으로 옮겨감)와 게임별 6줄(`score/minigame:*`). 게임별은 하루 캡이 **게임과 무관한 전역 회차 슬롯**(`activity_ledger.source_ref='play:1'…`)이라 나눌 수가 없다. 게임별로 나눌 거면 줄을 되살리지 말고 슬롯 체계부터 다시 짤 것.
  - ⛔ **관리자 감사로그(`admin_audit`)는 없앴다 (2026-09-07 지시 · `20260907210000`)** — 16곳이 쓰고 53건이 쌓였는데 **보는 화면이 0곳**이었다(서버에 `auditList` 액션까지 있었지만 부르는 화면이 없었다). 그래서 "누가 남의 계정을 초기화했나" 에 답할 수단이 지금은 없다(되돌리기 전 값은 응답으로만 돌아간다). ⛔ 되살릴 거면 **보는 화면을 같이** 만들 것 — 쓰기만 있는 로그는 없는 것과 같다.
...
- **메인 검색 라우터 — 지도가 낡는 게 이 기능의 고질병이다(2026-08-25 점검)**: 검색어 → `route-query`(임베딩 앵커 → LLM 분류 → 키워드 폴백) → 페이지 이동. 규칙이 **네 곳**에 흩어져 있다: `route-query` 의 `DEST`·SYSTEM 프롬프트·responseSchema enum, `route-seed` 의 시드 문구, `Landing.tsx` 의 `VALID_DESTS`·`clientKeywordRoute`. **한쪽만 고치면 안 되고, 넷이 일치해도 안심할 수 없다.**
  ...
  - **"시험 보러 왔어" 는 `/plan` 이다(2026-09-22 지시)** — 옛 `take_exam → /exam` 인텐트를 없앴다. 메인에서 그 말을 치는 사람은 응시권을 든 사람이 아니라 일정·접수를 알고 싶은 사람이고, 응시권을 든 사람은 마이페이지 응시권 카드로 `/exam` 에 들어간다(같은 날 `/exam` 위쪽 큰 응시 버튼도 뺐다). 같은 점검에서 어디로도 못 가던 말들을 기존 목적지에 붙였다: 탈퇴·계정 삭제 → `/terms`(탈퇴 버튼이 거기 맨 아래) · 내 응시권·닉네임/국가 변경 → `/mypage/attempts` · 칭호·배지·코인 선물·친구 초대 → `/hub`. 목적지로 **일부러 안 만든 것**: `/test/record`·`/test/certificate`("내 점수" 와 못 가른다) · `/mypage/inquiry`(`/faq` 가 링크를 갖고 있다) · `/verify/:token`(토큰 없이는 열 화면이 없다).
...
- **방문 통계 (2026-08-31 · `20260831130000`)** — 관리자 **홈 대시보드**(좌상단 이름 클릭) 안의 '방문 통계' 섹션. 일별 방문자·조회수 추이 + 국가별·지역별·기기·브라우저·OS·많이 본 화면. 라우트가 바뀔 때마다 `App.tsx` 의 `<VisitTracker/>` 가 `track-visit` 를 부르고, 집계는 `visit_stats` RPC 한 방이 한다(`admin` 의 `visitStats` 액션).
  - ⛔ **IP 로 국가를 정하지 않는다.** 국가는 **브라우저가 제3자(ipwho.is)에게 직접 물어본** 두 글자를 실어 보낸 것이고, 서버는 그걸 그대로 받는다 — `src/lib/geo.ts` 의 2026-08-24 결정. **엣지 함수에서 `cf-ipcountry` 로 국가를 뽑는 쪽으로 바꾸지 말 것.** 대가는 광고차단기에 막힌 방문이 **'미상'** 으로 남는 것이고, 화면이 그 사실을 밝힌다. ⚠️ **IP 자체는 2026-09-09 부터 대역만 남긴다**(아래 '통계 대메뉴' 절) — 국가 판정과는 별개 축이라 한 덩어리로 보고 같이 옮기지 말 것.
...
- **관리자 '통계' 대메뉴 · 방문 로그 (2026-09-09 · `20260909120000`)** — 숫자 화면이 제품마다 흩어져 있어서(홈·유저관리·아레나·CARIS) "회원이 늘고 있나", "돈은 얼마나 버나" 에 답하려면 화면 서넛을 오가야 했다. **주제로** 모은 대메뉴를 새로 만들었다: 전체 개요 / 접속·유입 / 회원 / 학습·참여 / 자격검정 / 매출.
  - ⛔ **제품(아레나/CARIS)으로 다시 가르지 말 것.** 2026-08-11 주석의 *"각 제품 상세 대시보드는 그 대메뉴 안에 남긴다"* 는 2026-09-09 지시로 **뒤집혔다**. ⛔ **목록·처리 화면을 통계에 넣지 말 것**(응시 기록·접수·응시권·결제 목록) — 보는 곳과 만지는 곳의 경계가 없어진다.
  - ⚠️ '전체 개요' 는 홈 대시보드(좌상단 이름)와 **같은 화면 한 벌**이다. 베껴서 두 벌로 만들면 같은 날 숫자가 화면마다 달라진다.
  - **접속·유입은 그누보드식 접속통계 레퍼런스를 따랐다**(요약 / 기간별 / 기기·환경 / 유입경로 / 방문자 로그). ⛔ 메뉴를 열 칸으로 늘리지 말 것 — 시간별·일별·월별은 같은 표에 묶는 단위만 다르고, OS별·브라우저별도 같은 표의 다른 열이다.
  - **IP주소별은 원문 그대로 본다 (2026-09-09 지시 · `20260909130000`)** — 같은 날 마스킹(`185.93.89.*`)으로 넣었다가 **열었다**(`ip_masked` → `ip`, 형태 CHECK 제거). ⚠️ 그 사이에 쌓인 행은 가려진 채 남아 있어 표에 원문과 섞여 보인다 — 고장이 아니다.
    - ⛔ **보유기간·파기가 유일한 방어다** — 크론 `visit-history-purge`(`purge_visit_history`, 매일 03:40 KST). **개인정보처리방침의 "180일 보관 후 파기"(집계는 400일)와 이 숫자는 한 벌**이라 한쪽만 고치면 안 되고, 크론을 끄면 방침이 거짓이 된다.
    - ⛔ **방침도 같이 고쳐야 한다.** 7판에 "뒷자리를 가린 대역 형태로만 보관합니다" 라고 **공개했었다** — 여는 마이그레이션만 적용하고 방침을 두면 공개 문서가 거짓이 된다(8판에서 그 괄호를 지웠다). 되돌릴 때도 같이 고칠 것.
    - ⚠️ 헤더(`x-forwarded-for`)는 클라가 위조할 수 있어 **모양 검사**를 거친다(`clientIp`). 안 거르면 관리자 표에 아무 문자열이나 줄로 선다.
    - ⚠️ 재동의는 안 받았다(2026-09-09 지시). 방침 제2조 자동수집 항목에 **'접속 IP 정보' 가 처음부터 있었다**.
  - **회원 상세 탭 8개 (2026-09-18 · PPT `관리자 페이지 수정사항` 2페이지 · `20260918120000`)** — 유저정보 › 메모 › 1:1 문의 › CARIS › WORLD ARENA › 결제·환불 › 독려이력 › 로그인정보. 2026-09-09 의 '문의·메모' 한 탭을 둘로 갈랐고(내용은 그대로), CARIS·WORLD ARENA 탭에 **문항별 정오답**(제출답안·응시기록 화면의 뷰어를 표 아래 줄에 펼친다 — 모달 안이라 모달을 또 안 띄운다), 결제 탭에 **환불 원장**(`refundList`)을 붙였다. 머리줄에 회원이 온보딩에서 고른 **국가·지역**도 같이 뜬다(⚠️ 방문 통계의 국가와 출처가 다르다).
    - **유저정보**(`memberInfo`) = 프로필·진척·코인·캐릭터·칭호 + **순위**. 순위는 랭킹 화면과 같은 RPC(`scoped_top`)로 서버가 센다 — 따로 세면 회원이 보는 숫자와 갈린다. 숫자 표로만 보여준다(2026-09-18 결정 — 랭킹 카드 그림을 그대로 넣지 않는다).
    - **독려이력**(`memberMailList`) = `mail_recipients`(사람별 줄 · `mail_log` 에 딸림) — 일시·종류·제목·받은 언어·발송됨/실패(사유)·보낸 사람. 2026-08-11~09-18 사이 `mailNudge` 는 이력만 남기고 실제로는 0통이 나갔다(그때 행은 status `logged`=미발송). ⛔ **화면에 개발 상태 문장("발송 서비스 미연결…")을 달지 말 것**(2026-09-18 질책 — "홈페이지가 니 주석창인줄 아는거야?"). 안 되는 건 오류 메시지로 그 순간에만 말한다.
  - **홈페이지 관리 › 메일 관리 (2026-09-22 지시)** — 독려 메일 세 벌의 **기본 문구**(제목·본문)와 **보내는 사람**을 고치는 유일한 화면(`MailAdmin`). 각 메일 창은 보낼 때 여기 저장된 값으로 시작하고 창에서 고친 건 그 한 번뿐이다. 메일 정의(종류·템플릿 키·치환자·미리보기 값·광고 여부)의 단일 출처는 **`src/lib/mailTemplates.ts`** — 메일 관리 화면과 메일 창 셋이 같은 표를 본다(두 벌이면 치환자 하나를 늘릴 때 한쪽만 늘어난다). 발신자 칸은 사이트 정보에서 여기로 옮겼다.
  - **메일 실제 발송 = Resend 한 곳 (2026-09-18 결정 · `_shared/mail.ts` · `20260918160000`)** — 세 화면(시험환경 점검·레벨테스트·응시권 미사용자)이 전부 관리자 함수의 `mailNudge` 하나로 온다. 설정 둘: 시크릿 `RESEND_API_KEY`(2026-09-22 등록) + 발신자(홈페이지 관리 › 메일 관리 `sender_name/sender_email` = `CARIS <noreply@garacaris.com>`, 없으면 `MAIL_FROM`). 도메인 `garacaris.com` 은 Resend 가 Cloudflare 에 레코드를 직접 넣어 인증했다(2026-09-22). **둘 중 하나라도 없으면 아무것도 기록하지 않고 400** — 안 보낸 걸 보냈다고 남기지 않는다. ⚠️ 도메인 DNS 인증(SPF/DKIM)은 Resend 가 주는 레코드를 우리 도메인에 넣어야 한다(2026-09-18 현재 **미완** — 도메인은 있다고 함, 이름·키는 나중에).
    - **번역 발송**: 사람마다 언어 = 마지막 레벨테스트 응시 언어 → 마지막 CARIS 응시 언어 → 국가(`langForCountry`, `zh-Hans` 는 `zh` 로 접음) → 한국어. 외국어 대상이 하나라도 있으면 **번역기 호출은 1번**(`deps.translateKoFields` 가 한 요청으로 5개국어를 준다) — 100명이어도 1번, 한국어만이면 0번. ⛔ 사람마다 번역하지 말 것. 치환자(`{name}` 등)와 줄바꿈은 `splitTemplate`/`joinTemplate` 으로 번역 바깥에 두었다가 사람마다 `fillVars` 로 끼운다 — 번역기가 `{name}` 을 못 건드린다. 번역기가 죽으면 전원 한국어로 나간다(발송은 안 막는다).
    - ⛔ **광고성 메일은 광고 수신 동의자에게만 (2026-09-22 결정 · 정보통신망법 §50)** — 종류별로 가른다: `nudge_leveltest`(레벨 UP 독려 = "다시 와서 해보라" 권유 = 광고)는 `profiles.marketing_agreed_at` 있는 사람만, `nudge_env_check`·`nudge_ticket`(본인이 접수한 시험의 점검·마감 고지 = 서비스 안내)은 접수자 전원. 셋 다 동의자로 묶으면(A안) 동의자가 극소수라 접수자에게 마감을 알릴 수단이 없어진다 — 그래서 B안. 검사는 `mailNudge` 안(우회 방지 · 응답 `noConsent`) + 응시자 탭의 '광고 수신' 열(미동의자는 **체크박스 잠금**, 버튼 비활성화는 관리자가 누가 문제인지 줄을 훑어야 해서 안 씀). ⚠️ 첫 발송(9/22) 4통 중 3통이 미동의자에게 나갔다 — 이 검사가 없어서였다.
    - ⚠️ **보낸 뒤 목록을 다시 받는다**(`MailComposeModal` 의 `onSent`) — 안 받으면 '마지막 독려' 가 새로고침 전까지 비어 있어 관리자가 "안 눌렸나?" 하고 같은 사람에게 또 보낸다(2026-09-22 지적). 창 안의 보내기 버튼도 한 번 보내면 잠긴다.
    - **발송**: 100통씩 묶음(Resend batch). 묶음은 주소 하나가 틀려도 통째로 거절되므로, 거절되면 그 묶음만 한 통씩 다시 보내 **실패한 주소만** `failed`+사유로 남긴다. 같은 주소는 한 통(응시권 둘 가진 사람). 받은 뒤 반송은 모른다(웹훅 미연결). 화면은 사람마다 채울 값을 `targets[].vars` 로 같이 보낸다 — 미리보기와 실제 발송이 같은 값.
    - 검증 = `tests/mail-send.mjs`(15건 — ⭐치환자·줄바꿈 보존, ⭐묶음 거절 → 실패 주소만 실패, 발신 설정 판정). 배포 = 마이그레이션 + `admin` 배포 + 프론트 push + 시크릿 `RESEND_API_KEY` 등록.
    - **로그인정보**(`memberAccess`) = 인증 서버의 마지막 로그인 + **방문 로그를 날짜 × IP × 기기로 접은** 접속 기록(180일). 로그인 이벤트 표는 따로 안 만들었다(2026-09-18 결정). ⚠️ 로그인 전 방문은 그 사람 것으로 못 센다(방문 로그의 회원 ID 는 로그인한 채 본 화면에만 붙는다).
    - 검증 = `tests/db/t-mail-recipients.mjs`(10건). 배포 = 마이그레이션 + `npx.cmd supabase functions deploy admin`(플래그 없이) + 프론트 push.
  - **레벨테스트 '응시자' 탭 + 독려 메일 (2026-09-18 · PPT 4페이지 · `20260918130000` → 2026-09-21 `20260921120000`)** — WORLD ARENA › 레벨테스트 › **응시자**(3단 맨 앞 = 기본 화면). 사람 = 한 줄(이름·이메일·레벨테스트 등급·응시 횟수·마지막 응시·경과·마지막 독려), 필터 '마지막 응시 후 N일'(0 = 전원), 체크해서 「독려 메일」. 처음(9/18)엔 응시 기록 탭에 버튼 + 모달이었는데 **응시 기록은 "시험 한 건 = 한 줄" 이라 사람을 고르는 자리로 어색해서** 탭으로 뗐다(2026-09-21 지시). 대상 = SQL `leveltest_nudge_candidates`(게스트·탈퇴자·무응시자 제외 · 응시 횟수 포함). 메일 창은 시험환경 점검과 **같은 컴포넌트**(`MailComposeModal` — `kind`·템플릿 키·치환자만 다르다). 템플릿 기본값은 `site_settings` 의 `mail_leveltest_subject/body`(치환자 `{name}`·`{level}`·`{link}`). ⚠️ 여기 '레벨'·`{level}` 은 **레벨테스트 등급**(시험 사다리)이지 시즌 점수의 ARENA 레벨이 아니다.
    - ⛔ PPT 의 "N일 후 **자동** 발송" 은 안 만들었다 — 스케줄러가 없다(pg_cron 은 HTTP 를 못 부른다 · 2026-09-18 결정: 수동 선택 발송). 번역 발송(③)은 `mailNudge` 안에서 된다(위 '메일 실제 발송' 절).
    - ⚠️ 창의 '마지막 독려' 열 + '최근 N일 안에 독려받은 사람 제외'(기본 켬)가 있는 이유 = 지난주에 보낸 사람이 이번 주에도 후보로 뜬다. 판정은 `mail_recipients` 의 `nudge_leveltest` 줄.
    - 검증 = `tests/db/t-leveltest-nudge.mjs`(13건). 배포 = 마이그레이션 + `admin` 배포 + 프론트 push.
  - **응시권 미사용자 독려 메일 (2026-09-18 · PPT 6페이지 · `20260918150000`)** — 접수·응시권 › 응시권 목록의 「미사용자 메일」(회차를 골라야 켜진다). 그 회차의 미사용 응시권(`examTicketList` status=issued, 응시 창이 끝난 것은 화면값으로 뺀다)을 골라 같은 메일 창(`kind='nudge_ticket'`, 템플릿 `mail_ticket_subject/body`, 치환자 `{name}`·`{round}`·`{tier}`·`{examEnd}`·`{link}`)으로. ⚠️ 같은 사람이 급수 둘을 접수했으면 응시권이 둘이라 **userId 로 접어 사람당 한 통**. ⚠️ PPT 문구에 마감일이 없어 "응시 마감: {examEnd}" 한 줄을 넣었다(지시 "넣어"). 서버 변경 = `MAIL_KINDS` 에 종류 추가뿐.
  - **시험환경 점검 독려 본문 교체 (2026-09-18 · PPT 5페이지 · `20260918140000`)** — `site_settings.mail_nudge_body` 기본값을 PPT 문구로 갈았다(제목은 그대로). ⚠️ PPT 문구가 "아래 기한 내에" 라면서 기한이 없어 **"점검 기한: {examDate} 전날까지"** 한 줄을 넣었다(지시 "넣어"). 코드 변경 없음 — 마이그레이션만 적용하면 된다.
    - ⛔ **메모는 본인에게 안 나간다.** RLS 정책 0개 + `memberNote*` 세 액션이 유일한 통로다. 사용자 화면에서 읽는 경로를 만들지 말 것.
    - ⛔ **보는 화면과 같이 만들었다** — `admin_audit` 을 없앤 이유(쓰기만 있고 보는 화면이 0곳)를 되풀이하지 말 것.
    - ⚠️ 문의 답변은 이 탭에서 **안 한다**(읽기 전용). 답변은 게시판 관리 › 고객센터 › Q&A 한 곳에서만 — 두 자리에서 쓰면 초안·상태 처리가 갈린다. 목록은 기존 `inquiryList` 에 `userId` 를 얹어 쓴다(액션을 새로 만들면 응답 모양이 갈린다).
  - **새 표 `visit_log` 는 요약표를 대체하지 않는다** — 요약(`visit_events`)은 "몇 명이 왔나"를 싸게 세고, 로그는 "한 건 한 건이 무엇이었나"(시간별·유입경로·최초 접속 페이지·방문자 로그)를 본다. ⛔ **`track-visit` 가 둘 다 쓴다** — 한쪽만 부르면 같은 기간을 두고 두 화면이 다른 숫자를 말한다.
    - ⛔ **로그는 행 수에 바닥이 없다**(요약표의 그 불변식이 여기엔 없다). 대신 `visit_log_add` 안에서 **한 방문자 하루 1000줄** 상한을 건다 — anon 키로 부르는 쓰기 경로라 방어가 필요하다. ⚠️ 보존 크론은 아직 없다(정리 한 줄은 마이그레이션 머리 주석).
    - ⛔ **referrer 는 그 방문(탭)의 첫 요청에서만 받는다**(`sessionStorage` 판정). SPA 라 화면을 옮겨도 `document.referrer` 가 안 바뀌어서, 매번 보내면 **외부 유입 1건이 그 사람이 본 화면 수만큼 뻥튀기**된다. 그래서 유입경로 표의 합계는 다른 표보다 작은 게 정상이다.
  - **학습·참여 › 레벨테스트 통계는 게스트를 뺀다 (2026-09-22 지시)** — 게스트(익명 세션) 응시는 등급을 안 매겨서(`rank_dir` 비어 있음) 섞으면 전부 '유지' 로 세어져 등급 변동이 부풀고, 응시 언어·정답률·문항 난이도에도 소음이 된다(프로필 189 중 게스트 165 실측). `admin-test` 의 `analytics` 가 프로필 `is_anonymous` 로 응시·답안을 거르고 '게스트' 칸만 게스트 수를 보여준다. 미니게임·DAILY QUIZ 는 서버가 게스트를 거절해서 원래 소음이 없다. ⛔ **자격검정 탭의 '가입 추이' 는 뺐다** — 사이트 전체 가입이라 회원 통계 것이었다(8/11 제품 대시보드 잔재).
    - ⛔ **PostgREST 는 한 요청을 1,000행에서 자른다**(`max-rows`) — `.limit(10000)` 을 적어도 1,000 이다. 레벨테스트 분석이 답안 8,460건 중 1,000건으로 문항 난이도를 매기고 있었다. 지금은 `allRows`(기본키 정렬 + `range` 쪽 넘김)로 끝까지 받는다. **다른 통계 액션의 `.limit(N)` 도 같은 함정**이다(`cbtAnalytics` 등) — 행을 받아 세는 곳은 전부 의심할 것. 정확한 합계만 필요하면 `{ count: 'exact', head: true }`.
  - 회원·매출 통계는 새 액션 `memberStats`·`revenueStats` 다. ⚠️ 매출의 '미지급·회수필요' 두 줄은 **기간과 무관한 전체 누계**다 — 기간으로 자르면 오래된 미지급이 화면에서 사라진다.
  - 배포: 마이그레이션 + `npx.cmd supabase functions deploy track-visit admin`(둘 다 플래그 없이) + 프론트 push. 검증 = `tests/db/t-visit-log.mjs`(25건).
- **채팅 제재 사다리 (2026-09-10 지시 · `20260910120000`)** — 옛 상태: `profiles.suspended_until`·`suspended_reason` 과 `suspendUser` 액션이 2026-08-11 재편 때 만들어졌는데 **양쪽이 다 끊겨 있었다** — 거는 버튼이 화면에 없었고(src 전체 호출 0건) 읽는 코드도 0곳이라, 정지를 걸어도 아무것도 안 막혔다(실사용 0건). 그래서 신고를 받아도 관리자가 할 수 있는 건 *글* 처리(숨김·완전삭제·문제없음)뿐이었고 **쓴 사람은 아무 일도 안 당해서** 지우자마자 또 썼다. 지금은 신고 큐의 **[제재] 한 번**이면 끝이고 기간은 사다리가 정한다.
  - **사다리 = 1차 1일 · 2차 3일 · 3차 7일 · 4차 30일 · 5차 90일 · 6차 영구.** 90일 무위반이면 한 칸 내려간다(`유효차수 = 총 제재수 − floor(경과일/90)`). 근거는 게임사 관례 — 메이플스토리 채팅 제재가 3·7·15·30, 넷마블 1·3·7·365, RPLAY 3·7·15·30·90·180·영구.
  - ⛔ **사다리의 단일 출처는 SQL `chat_sanction_days()` 하나다.** 화면이 "다음 위반 시 30일" 을 보여주려면 그 숫자가 필요한데, 프론트에 표를 복사하면 sync pair 가 생긴다 — 서버가 `nextDays` 로 **계산해서 내려주고** 화면은 받아 적기만 한다.
  - ⛔ **기간을 요청으로 받지 않는다.** 관리자가 고를 게 없는 것이 이 설계의 요점이다(고르게 하면 관리자마다 다른 잣대가 되고 사다리가 있을 이유가 없다). 같은 이유로 '기간만 줄이기' 도 없다 — 취소(`chat_sanction_revoke`)는 **오판 정정용**이라 마지막 기록을 지우고 차수까지 되돌린다.
  - ⛔ **막는 것은 채팅뿐이다.** 읽기·시험·결제·응시권은 그대로다(원래 의도가 채팅 입막음이고, 돈 낸 걸 막으면 별도 정책이 필요해진다). 검사는 `chat-post` 한 곳 — `resolvePoster` 가 **어차피 읽는 profiles 행에 칸만 얹어서** 쿼리를 안 늘린다.
  - ⛔ **사유는 코드 6개다**(`spam`·`abuse`·`sexual`·`flood`·`privacy`·`other` — 신고 사유와 같은 값). 자유 텍스트로 받으면 아레나가 6개국어인데 **베트남 사용자가 관리자의 한국어 사유를 그대로 본다.** 코드면 사전(`chat.report*`)이 이미 6개국어를 갖고 있다. 관리자 메모(`note`)는 내부용이라 사용자에게 안 나간다.
  - ⛔ **`chat_sanctions` 에 본문을 담지 않는다.** 완전삭제의 존재 이유가 "그 본문이 어딘가 남아 있는 것 자체가 문제(개인정보·불법물)" 라, 복사해두면 지운 게 아니게 되고 파기 요청 때 이 표까지 뒤져야 한다. `message_id` 도 `on delete set null` — **글을 물리 삭제해도 제재 사실(사유·차수·시각)은 남는다.** 옛 구조의 구멍이 정확히 이거였다(제일 악질인 완전삭제 케이스가 흔적 0이었다). 이의 제기 대응엔 사유+차수+시각이면 충분하다.
  - ⛔ **자동 가림(신고 3명)은 차수에 안 넣는다.** 넣으면 서로 짜고 3명이 신고하는 것만으로 남을 6차 영구정지로 몰 수 있다. 관리자 화면엔 '자동가림 N회' **참고 숫자**로만 뜬다(회색으로 따로 뗀다 — 섞이면 짜고 신고한 결과가 제재 근거처럼 읽힌다). 자동 가림의 성격은 **임시조치**다(관리자가 숨김/문제없음을 누르기 전까지의 잠정 상태).
  - ⚠️ **영구는 `until = 9999-12-31`** 이다. 그래야 검사가 `suspended_until > now()` **한 줄로 통일**되고 영구든 3일이든 같은 코드가 막는다(화면만 연도 9999 를 '영구' 로 읽는다).
  - ⚠️ 정지된 사람은 **입력창 자리에 안내**를 본다(언제까지·왜·몇 차·다음 단계·[이의 제기]→`/feedback`). 입력창을 흐리게만 두면 "왜 안 써지지" 로 남는다. **다음 단계를 예고하는 건 의도**다 — 억제 효과가 있고, 영구정지를 당했을 때 '못 들었다' 가 안 된다.
  - ⚠️ 옛 `suspendUser`(reform.ts)는 **지웠다.** 기간·사유를 요청으로 받아 `profiles` 를 직접 써서, 두면 사다리를 우회하는 두 번째 경로가 되고 `chat_sanctions` 의 차수와 실제 정지가 갈린다. 걸 수 있는 길은 하나여야 한다.
  - ⚠️ '이 사람 글 전부 숨기기' 는 제재 모달의 체크박스이고 **기본 꺼짐** — 도배는 신고 1건에 글이 30개라 필요하지만, 욕설 한 마디에 켜면 그 사람의 멀쩡한 옛 글까지 사라져 남들 대화 맥락이 뚫린다. 반대로 완전삭제의 '제재도 함께' 는 **기본 켜짐**(보통은 남의 개인정보 유포다. 본인이 실수로 자기 연락처를 올려 지워달라는 경우에만 끈다).
  - 배포: 마이그레이션 + `npx.cmd supabase functions deploy chat-post chat-list admin`(전부 플래그 없이) + 프론트 push. 검증 = `tests/db/t-chat-sanctions.mjs`(28건 — ⭐사다리 값·⭐90일 감면·⭐profiles 갱신·⭐취소가 차수까지 되돌림·⭐본문 컬럼 없음).
- **채팅 모더레이션이 죽어도 채팅은 돈다 (2026-09-10 지시)** — 옛 기본값은 fail-closed(`CHAT_MOD_FAILCLOSED=true`)라 **OpenAI 가 30분 죽으면 채팅도 30분 죽었다.** 게다가 장애 기록 insert 가 fail-open 갈래 안에만 있어서 기본 설정에서는 **기록조차 안 남았고**, `chat_incidents` 는 읽는 코드가 0곳이었다. 지금은 기본이 fail-open 이다.
  - ⛔ **`chat-list` 의 노출 필터에서 `pending` 을 빼지 말 것**(`VISIBLE_MOD = ['ok','pending']`). 옛 fail-open 은 글을 `pending` 으로 넣었는데 노출은 `ok` 만 통과시켜서, **글은 써지는데 아무도 못 보는** 상태였다 — 사용자에게는 채팅이 죽은 것과 구별되지 않아 토글에 아무 쓸모가 없었다. `auto_hidden`(신고 3명)은 계속 빠진다.
  - OpenAI 하나가 빠질 뿐 **나머지 관문 다섯은 그대로 돈다** — 금칙어(코드+`banned_words`) · 링크 차단 · 도배 제한 · 신고 3명 자동 가림 · 관리자 제재. 그것 때문에 채팅을 통째로 멈추는 게 손해가 크다는 판단이다.
  - ⛔ **장애 알림은 `system_alerts` 로 보낸다 — `chat_incidents` 에 새로 쌓지 않는다**(옛 행은 그대로 둔다). 표가 둘이면 또 한쪽을 안 보게 된다. 관리자 홈 대시보드가 이미 그 표를 그리고 있고, `dedupe_key='chat-mod-down'` 의 부분 유니크 인덱스 덕에 **중복 insert 가 23505 로 튕기는 것이 곧 중복 제거**다(100번 터져도 한 줄). 복구되면 `clearModOutage` 가 `status='resolved'` 로 닫는다 — 안 닫으면 한 번 뜬 알림이 영구히 남아 아무도 안 본다. ⚠️ 알림은 관리자 홈에만 간다(2026-09-10 선택) — **밤에 터지면 다음 `/admin` 접속 때 본다.**
  - ⛔ **복구 재검사에 크론을 만들지 않았다.** pg_cron 은 SQL 만 돌려 OpenAI 를 못 부르고, 부르려면 확장(pg_net)을 새로 깔아야 한다. 대신 **다음 사람이 글을 쓸 때 곁다리로** 돈다(`recheckPending`) — "방금 OpenAI 가 정상 응답했다" 가 복구를 아는 가장 확실한 신호다. 채팅이 조용하면 재검사도 안 돌지만, 조용하면 pending 도 안 쌓이므로 어긋나지 않는다. 재검사에서 flagged 면 원래대로였으면 422 였을 글이므로 내린다(물리 삭제 아님 — `hidden_by='admin'` 이라 '숨김 해제'로 되살릴 수 있다).
    - ⚠️ **"장애를 본 적 있으면 그때만" 으로 만들지 말 것** — 엣지는 인스턴스가 여럿이라 장애를 본 인스턴스와 복구 후 첫 글을 받는 인스턴스가 다를 수 있고, 그러면 pending 이 영영 안 풀린다. 그래서 모듈 전역 타이머(`SWEEP_MS` 60초)로 **인스턴스당 주기적으로** 돈다(겹쳐 돌아도 무해하다).
  - ⚠️ 화면 타입에 `'auto_hidden'` 이 오래 빠져 있었다(`ChatBoard.tsx` 의 `ModStatus`) — 서버는 보내는데 타입에 없어서 화면이 그 상태를 아예 몰랐고, **자기 글이 남에게 안 보인다는 사실을 작성자가 알 방법이 없었다**(반응이 없으니 또 쓰고 신고가 더 쌓였다). 지금은 배지가 뜬다: "다수의 신고가 접수되어 검토 중입니다. 지금 이 글은 나에게만 보입니다." ⛔ **몇 명이 신고했는지는 밝히지 않는다** — 임계치가 새어나가면 그 수를 맞추려는 조직적 신고를 유도한다.
  - ⚠️ 도배 제한(`chat_post_atomic` 의 4가드)은 **손대지 않았다**(2026-09-10 결정). 창이 전부 60초라 61초마다 쓰면 안 걸리는 구멍이 있지만, 일일 상한을 추측으로 걸면 정상 사용자를 먼저 때린다 — 느린 도배는 신고 → 제재 사다리로 잡는다.
...
- **허브 캐릭터 업로드 (2026-08-31 · `20260831140000`)** — 관리자 › WORLD ARENA › 마이홈 › 꾸미기 관리 › **캐릭터 업로드**. 캐릭터를 늘리는 데 **배포가 필요 없다**(2026-08-20 의 "그림은 코드, 가격은 DB" 선을 캐릭터에서만 옮긴 것 — 캐릭터가 가진 수치는 비율 하나뿐이라 그림에서 재면 되기 때문이다. 스킨은 9패치 값 15줄이 그림과 같이 와야 해서 여전히 배포다).
  ...
  - ⛔ **그림의 단일 출처는 `hub_char_art` 표다 — 옛 코드 파일(`public/hub/char/<키>/lv<n>.webp`)은 2026-09-15 에 지웠다(지시).** 처음(8/31)엔 `char_a_m`·`char_a_f` 가 표에 없어 코드 파일이 유일한 출처라 "지우지 말 것" 이었는데, 그 뒤 6종 전부 올라가서 이유가 없어졌다. 남겨 두는 동안 **표가 도착하기 전 한 박자 옛 그림이 먼저 떴다가 올린 그림으로 바뀌는** 번쩍임만 만들고 있었다. 표에 없는 키는 폴백 한 장(`CHAR_FALLBACK_SRC`, 캐릭터를 아직 안 고른 무대와 같은 그림)이다. ⚠️ `tools/build-char-art.mjs` 가 만드는 `public/hub/char/` 는 **업로드용 중간 산출물**이라 커밋하지 말 것.
...
- **`/daily`·`/games` i18n (2026-08-07)**: 화면 문구는 `daily.*`·`mg.*` 로 이관했다. 미니게임 제목·소개·뱃지는 `lib/minigames.ts` 에서 빼고 **`mg.<id>.title|tagline|badge` 키 조립**으로 바꿨다(데이터에 한국어를 두면 언어를 바꿔도 커버만 한국어로 남는다).
  - **게임 안 문구도 번역된다 (2026-08-07)**: 게임은 자립형 HTML(iframe)이라 앱 사전을 못 쓴다 → 부모가 `iframe src` 에 `?lang=` 을 붙이고, **공용 사전 `public/games/i18n.js`** 가 그걸 읽어 갈아끼운다. HTML 은 `data-i18n="키"`(원문은 그대로 두어 사전이 없어도 한국어로는 보인다), JS 생성 문구는 `MGI18N.t('키', {n})`. 문서 끝에서 `MGI18N.apply()` 한 번.
    ...
    - 적용 완료 = 용어 퀴즈 3종(`beat-cari`·`shoot-cari`·`pick-cari`) + 닿아라(`reach-cari`)·프로그램해라(`program-cari`)(2026-09-14)·지어라(`build-cari`)·막아라(`block-cari`)·시켜라(`order-cari`)(2026-09-15)·더듬어라(`feel-cari`)(2026-09-16) — **9종 전부**. 레벨 문구는 `reach.lv{n}.*`·`prog.lv{n}.*`·`build.lv{n}.*`·`order.lv{n}.*`·`feel.z{n}`·서류는 `block.d{일차}.{번호}.*` 키, 레벨을 넣으면 사전도 같이.
  - **게임 문항이 DB 로 모였다 (2026-09-03 · `20260903120000`)** — 옛 상태: 같은 50문항이 게임 HTML 3벌의 `POOL` 로 복제돼 있어 문항 하나를 고치려면 개발자가 파일을 고치고 배포해야 했고, `term_questions` 표와 관리자 화면은 2026-08-11 에 만들어졌는데 **아무도 안 읽어서** 저장만 되고 게임에는 안 나갔다.
    - **DAILY QUIZ 는 같은 표의 다른 은행이다(2026-09-08 지시 · `20260908120000`).** 은행 = `term_banks` 두 줄(a1 게임 · a2 DAILY) + `term_questions.bank_id`(NOT NULL · 기본 a1). 관리 화면은 `AdminTermQuestions` 한 벌이 `bank` prop 으로 갈라 선다(관리자 › DAILY QUIZ › **문항 관리**, 번호 `D-###`). 매핑의 단일 출처 = `_shared/term-banks.ts`(admin·term-pool 공용).
      - ⛔ **2026-09-03 에 "DAILY 는 이 은행을 안 쓴다" 로 뗐던 이유**는 게임 은행을 같이 읽으면 관리자가 게임 문항을 고칠 때 DAILY 도 따라 바뀌어서였다. 은행을 갈랐으니 그 이유가 없어졌다 — ⛔ 되돌려서 `'daily'` 를 **게임 은행에** 물리지 말 것.
      - ⛔ **은행을 보는 자리 다섯(목록·되돌리기 목록·이력·번호 매기기·중복 검사)은 전부 `bank` 로 거른다.** 빠뜨리면 DAILY 탭에 게임 문항이 뜬다 — 에러 없이 조용히 섞인다. id 로만 만지는 것(수정·중지·삭제·번역 저장)은 은행이 필요 없고, 이력의 `scope` 는 **요청의 bank 가 아니라 그 행의 bank_id** 로 적는다(`bankOfRow`).
      - ⛔ **정답 유일 인덱스는 은행 안에서만**(`(bank_id, answer_i18n->>'ko') where active`). 같은 용어(머신러닝)가 두 은행에 있는 게 정상이다 — 전역으로 되돌리면 DAILY 시드가 통째로 막힌다. 이름은 그대로(`term_questions_answer_uniq`, 서버가 그 이름으로 오류를 옮긴다).
      - ⛔ **시드 = `src/lib/terms.ts` 의 50개 그대로, 같은 순서.** 마이그레이션만으로 오늘의 문제가 바뀌면 안 된다(순환 인덱스가 `sort_order → code` 순서 위에서 돈다). 게임 은행을 복사하지 않은 이유: 관리자가 그쪽 문장을 손봤고 9개를 지웠다. 번역은 게임 은행에서 **한국어 세 칸이 정확히 같은** 살아 있는 행의 것만 복사했다 — 나머지는 관리자의 「🌐 미번역 번역」 한 번. 검증 = `tests/db/t-term-banks.mjs`(시드 ↔ terms.ts 50개 전부 대조).
      - ⚠️ **이력(`question_history`, kind=`term`)의 `scope` = 은행 키(`game`·`daily`).** 옛 159행은 마이그레이션이 전부 `game` 으로 채웠다. 한 kind 안에 은행이 둘이라 `scope` 까지 걸어야 남의 은행 이력이 안 섞인다.
      - **해설(글+그림)은 코드에 있고, 묶음 파일로 나눠 쓴다(2026-09-08).** 글 = `src/lib/theory/batch*.ts`(`terms.ts` 의 `TERM_THEORY` 가 합친다) · 그림 = `src/components/dailyVisuals/batch*.tsx`(`DailyVisual.tsx` 의 등록표가 합친다) · 공용 부품 = `dailyVisuals/kit.tsx`(`Frame`·`Limb`·`Joint`·`Box`·`Arrow`·`Lab`·`Chip`). 열쇠는 여전히 **한국어 정답 표기**다.
        - ⛔ **묶음을 만들었으면 합치는 자리에 꽂아야 한다** — 안 꽂으면 해설에 그림만 조용히 빠지거나 그 용어가 화면에 안 나온다. 빠뜨림·키 충돌·색 하드코딩은 `tests/daily-theory.mjs` 가 잡는다(`test:db` 포함).
        - ⛔ **그림에 색을 직접 쓰지 말 것**(`fill="#333"` 금지). 선은 `stroke="currentColor"`, 면은 `.vz-*` 클래스 — daily.css 토큰만 쓰므로 다크/라이트가 자동으로 따라온다. 박으면 한쪽 테마에서 그림이 배경과 같은 색이 되어 증발한다.
        - ⚠️ **그림 키는 묶음 접두사(`b3_`…)로 시작한다.** 등록표는 하나로 합쳐지므로 키가 겹치면 뒤엣것이 앞엣것을 덮는다.
        - ⚠️ 그림을 훑는 자리는 **`/daily/gallery`**(개발 서버 전용 도감 · 빌드에는 라우트가 없다). `/daily` 는 하루 한 장이라 검수가 안 된다.
        - **해설도 6개국어다(2026-09-08).** 덮어쓰기 표를 언어별 파일로 둔다 — `src/lib/theory/i18n/<lang>.labels.ts`(그림 속 글자 442개) + `<lang>.theory.ts`(해설 글 127개), 둘을 `<lang>.ts` 가 묶고 `lib/dailyI18n.ts` 가 그 언어를 고른 사람에게만 받아 온다. **키는 한국어 원문**이고, 표에 없으면 한국어가 그대로 나온다(번역이 덜 된 언어도 화면이 안 빈다).
          - ⛔ **해설을 찾는 열쇠는 `answerKo`(한국어 정답)다.** 화면에 보이는 정답은 언어로 투영돼 있어서, 그걸로 찾으면 **외국어에서만 해설 카드가 통째로 안 뜬다**(2026-09-08 실측). `term-pool` 이 `answer`(투영)와 `answerKo`(원본)를 같이 내려주는 이유다.
          - ⛔ **그림 속 글자는 반드시 `Lab` 부품을 거친다 — 날 `<text>` 금지.** 거기 한 곳에서 갈아 끼우므로, 날 `<text>` 로 쓰면 그 글자만 영영 한국어로 남는다(실제로 13곳이 그랬다). 글자와 식이 섞이는 자리는 `tLab('…')` 로 조각만 번역한다.
          - ⚠️ **번역은 길이가 문제다.** 그림은 가로 320 고정 상자에 좌표를 박는다 — 한국어 4자가 영어 10자가 되면 도형을 뚫는다. `tmp/daily-gen/overflow-check.mjs` 가 언어별로 실제 렌더 폭을 재서 넘치는 것을 잡는다(도감의 언어 버튼으로 눈으로도 본다).
          - ⚠️ 번역표에 **원문에 없는 키**가 남아 있으면 그 번역은 죽은 값이다(원문을 고친 자리) — `tests/daily-theory.mjs` 가 잡는다.
        - ⚠️ **그림 묶음은 통째로 한 청크다**(`daily-*.js` · 127장에 198KB/gzip 53KB). `/daily` 를 열면 그날 쓰는 한 장 때문에 전부 받는다 — 첫 진입(`index`)에는 안 들어가므로 지금은 감당되지만, 그림이 더 늘면 **그림 단위 lazy 로 쪼갤 자리**다. 해설 글(`terms-*.js` 36KB)은 따로 떨어져 있다.
      - **해설 그림이 있는 문항만 은행에 산다(2026-09-08 지시 · `20260908130000`).** 그날 해설 없는 42문항을 지웠고, 이후 `20260908140000` 이 해설이 붙은 것들을 되살리고 새 문항을 넣었다. ⛔ 되살릴 때 **본문은 한 글자도 안 건드린다** — 한국어가 바뀌면 그 행의 번역을 비우는 게 규칙이라, 6개국어가 찬 행을 다시 쓰면 번역 5벌을 버리게 된다. 나머지 42문항은 삭제 탭에서 되돌릴 수 있다. ⛔ 마이그레이션의 8개 이름 ↔ `TERM_THEORY` 키가 sync pair 다(`t-term-banks.mjs` 대조). 관리자 › DAILY QUIZ › 문항 관리에 **해설 미리보기**(목록 '해설' 열 · 수정 모달 하단)가 있고, `/daily` 와 같은 카드(`components/DailyTheoryCard`)를 `.dy-page.dy-embed` 로 감싸 그린다 — 보기만 되고 편집은 코드다. 표기를 바꾸면 해설이 떨어지는 걸 미리보기가 그 자리에서 보여준다.
      - ⚠️ `/daily` 는 **자리(`dailyIndex`)와 보기 순서(`dailyShuffle`)가 언어와 무관**하게 같다 — 고른 보기를 문자열이 아니라 자리로 든다(언어를 바꾸면 다시 받으니까). 관리자가 문항을 빼거나 넣으면 **그날 문제가 바뀐다**(의도된 성질, 화면 힌트에 적혀 있다). 해설(`TERM_THEORY`)은 한국어 정답이 키라 은행에서 표기를 바꾸면 떨어진다.
    - 단일 출처 = **`term_questions`**. 서빙은 `term-pool`(대상 `gameId` → 은행, 화면 언어로 투영, 순서 `sort_order → code` 고정). 게임은 자립형 iframe 이라 부모(`MiniGame.tsx`)가 받아 `postMessage {t:'mg:pool'}` 로 주입하고, `/daily` 는 `fetchTermPool('daily')` 로 직접 받는다.
    - ⚠️ **게임 HTML 의 `POOL` 은 폴백이다** — 고쳐도 게임에는 안 나간다(서버가 안 열릴 때만 쓰인다). 게임 문항을 바꾸는 자리는 관리자 화면 하나다. (`terms.ts` 는 폴백이자 **DAILY QUIZ 의 진짜 출처**다.)
    ...
    - ⛔ **게임별로 문항을 고르는 기능은 없다(2026-09-03 지시).** 은행에 '사용' 상태로 있는 문항이 세 게임 전부에 나간다 — 셋은 같은 용어 문제를 **보여주는 방식만** 다르고(4지선다 / 운석 / O·X), 실제로도 50문항이 통째로 들어가 있었다. 안 쓰는 선택 UI 가 화면 한복판에 있으니 관리자가 화면을 못 읽었다. 한 문항을 빼려면 **'중지'** 를 누른다(세 게임에서 같이 빠진다). 표 `minigame_question_sets` 는 2026-09-04 에 드롭했다(`20260904540000`) — 남겨둔 200줄이 되살릴 때 쓸모가 없었다(되살리는 방법이 이미 "분야로 걸기" 로 정해져 있고 분야는 `term_questions.field` 에 있다 · `daily` 50줄은 가리킬 대상이 없어졌다). ⛔ 되살릴 거면 표를 다시 만들지 말고 **분야(AI·로봇·피지컬AI)** 로 걸 것.
    - 배포: 마이그레이션 + `npx.cmd supabase functions deploy term-pool admin`(**둘 다 플래그 없이** — anon 키가 실려 오므로 공개 예외가 필요 없다) + 프론트 push. ⚠️ **순서 = 마이그레이션 → 함수 → 프론트.** 프론트가 먼저 나가면 옛 서버가 `bank` 를 모른 채 게임 은행을 돌려줘 DAILY 탭에 게임 문항이 뜬다(반대 순서는 안전 — 옛 화면은 bank 를 안 보내고 서버가 게임으로 접는다).
- **문항 변경 이력은 세 제도가 한 표를 쓴다 (2026-09-04 · `20260904400000`)** — 옛 상태: 제도만 다르고 하는 일이 같은 표가 셋이었다(`cbt_question_events` 56행 · `question_events` 455행 · `term_question_events` 159행). 컬럼 6칸(id·question_id·action·actor·detail·created_at)이 완전히 같고 다른 것은 **"사람이 읽는 문항 이름표"와 "어느 묶음 소속인가"** 두 칸의 이름·타입뿐이었다. 새 표 **`question_history`** 가 그 둘을 `label`(옛 `number`·`code` — 120 / L3-045 / T-001) · `scope`(옛 `bank_id`·`level`)로 통일하고 `kind`(`caris`|`leveltest`|`term`)로 제도를 가른다.
