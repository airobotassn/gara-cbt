...
- **관리자 '통계' 대메뉴 · 방문 로그 (2026-09-09 · `20260909120000`)** — 숫자 화면이 제품마다 흩어져 있어서(홈·유저관리·아레나·CARIS) "회원이 늘고 있나", "돈은 얼마나 버나" 에 답하려면 화면 서넛을 오가야 했다. **주제로** 모은 대메뉴를 새로 만들었다: 전체 개요 / 접속·유입 / 회원 / 학습·참여 / 자격검정 / 매출.
  ...
  - **메일 실제 발송 = Resend 한 곳 (2026-09-18 결정 · `_shared/mail.ts` · `20260918160000`)** — 세 화면(시험환경 점검·레벨테스트·응시권 미사용자)이 전부 관리자 함수의 `mailNudge` 하나로 온다. 설정 둘: 시크릿 `RESEND_API_KEY` + 발신자(홈페이지 관리 › 사이트 정보 › 메일 발신자 `sender_name/sender_email`, 없으면 `MAIL_FROM`). **둘 중 하나라도 없으면 아무것도 기록하지 않고 400** — 안 보낸 걸 보냈다고 남기지 않는다. ⚠️ 도메인 DNS 인증(SPF/DKIM)은 Resend 가 주는 레코드를 우리 도메인에 넣어야 한다(2026-09-18 현재 **미완** — 도메인은 있다고 함, 이름·키는 나중에).
