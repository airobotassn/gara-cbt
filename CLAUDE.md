...
- **관리자 '통계' 대메뉴 · 방문 로그 (2026-09-09 · `20260909120000`)** — 숫자 화면이 제품마다 흩어져 있어서(홈·유저관리·아레나·CARIS) "회원이 늘고 있나", "돈은 얼마나 버나" 에 답하려면 화면 서넛을 오가야 했다. **주제로** 모은 대메뉴를 새로 만들었다: 전체 개요 / 접속·유입 / 회원 / 학습·참여 / 자격검정 / 매출.
  ...
  - **레벨테스트 독려 메일 (2026-09-18 · PPT 4페이지 · `20260918130000`)** — WORLD ARENA › 레벨테스트 › 응시 기록의 「독려 메일」. 대상 = **마지막 제출 응시가 N일 지난 회원**(SQL `leveltest_nudge_candidates` — 게스트·탈퇴자·무응시자 제외), N 은 창에서 입력(기본 7). 메일 창은 시험환경 점검과 **같은 컴포넌트**(`MailComposeModal` — `kind`·템플릿 키·치환자만 다르다). 템플릿 기본값은 `site_settings` 의 `mail_leveltest_subject/body`(치환자 `{name}`·`{level}`·`{link}`).
